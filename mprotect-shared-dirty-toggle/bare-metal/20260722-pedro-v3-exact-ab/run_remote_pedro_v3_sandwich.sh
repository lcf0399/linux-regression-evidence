...
BASE=${BASE:-$REMOTE_ROOT/linux-baremetal/mprotect-pedro-v3-exact-20260722}
...
POINT_RUNNER=./linux-regression-evidence/mprotect-shared-dirty-toggle/bare-metal/20260722-pedro-v3-exact-ab/run_pedro_v3_ab_point.sh

fail()
{
	echo "Pedro v3 remote sandwich failed: $*" >&2
	exit 1
}

ssh_remote()
{
	ssh -o BatchMode=yes -o ConnectTimeout="$CONNECT_TIMEOUT" "$HOST" "$@"
}
...
case "$START_AT" in
	parent-a)
		run_point parent parent-a
		run_point child child
		run_point parent parent-b
		;;
	child)
		run_point child child
		run_point parent parent-b
	...
	parent-b)
		...
		;;
	*)
		fail "START_AT must be parent-a, child, or parent-b"
		;;
esac

echo "sandwich_complete=yes"
echo "sandwich_design=parent-a,child,parent-b"
