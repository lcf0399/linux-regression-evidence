...
START_AT=${START_AT:-large-baseline-a}
HELPERS=./scripts/baremetal/kernel
POINT_RUNNER=${POINT_RUNNER:-./linux-regression-evidence/mprotect-shared-dirty-toggle/bare-metal/20260722-v713-shared-pte-hint-fastpath-ab/run_large_folio_point.sh}

POINT_ROLES=(baseline candidate baseline)
POINT_LABELS=(large-baseline-a large-candidate large-baseline-b)

fail()
{
	echo "large-folio remote matrix failed: $*" >&2
	exit 1
}
...
run_point()
{
	...
	ssh_remote "cd '$REMOTE_ROOT' && BOOT_SETTLE_SECONDS='$BOOT_SETTLE_SECONDS' bash '$POINT_RUNNER' '$role' '$label'"
...
echo "large_folio_matrix_complete=yes"
