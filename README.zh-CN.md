...
索引更新于 2026-09-10。当天加入 MSG_RING v3 补丁验证与生命周期补充，并核对协作者 v3 来信；futex stable
...
| [`io-uring-msg-ring-send-fd-install/`](io-uring-msg-ring-send-fd-install/) | 报告于 2026-08-07 发出并公开归档。协作者 Uzair Beg 于 2026-09-09 提供 v3，验证已完成；最近已发诊断回复仍为 2026-09-03 UTC。这不是维护者正式结论或合入确认。 | 原 `7029acd8a950` 精确 slowdown 仍为 `11.621%`。[原样 v3 在 v6.18-rc4 上](io-uring-msg-ring-send-fd-install/bare-metal/uzair-v3-prefill/README.zh-CN.md)使 first fill 对未修改版快 `9.791%`，同 ring 4096 文件移除→重装快 `18.820%`。注册至首填完成未改善；少量使用/小表的不稳定结果和闲置内存代价均保留。 |
...
| 证据 | 报告状态 | 当前技术状态 |
| --- | --- | --- |
| [`kernfs-empty-cgroup-removal/`](kernfs-empty-cgroup-removal/) | 原报告及 INODE_INITED 验证回复已发送；独立标记原型回复与完整补丁附件已于 9 月 17 日发送，等待审阅，不声称修复已被接受或合入 | 原精确删除增幅约 46%～48%。新[独立请求标记](kernfs-empty-cgroup-removal/bare-metal/inode-requested/README.zh-CN.md)在一轮矩阵中使删除耗时降低 22.36%～22.72%、连续整段降低 7.35%～9.01%，代价是每节点增加 8 字节。旧实验分开保留；正确性覆盖仍未齐，无应用级影响测量。 |
