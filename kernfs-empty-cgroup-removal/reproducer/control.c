...
struct result { uint64_t first, second, id, from, to; int weight, rejected; struct cg_stats stats; struct cg_task tasks[CG_MAX_TASKS]; };
...
int main(int argc,char **argv)
{
	...
	for(int i=negative?-1:0;i<rounds+warmup;i++){
		...
		if(family==1){
			uint64_t start=now(); int rc=mkdir(leaf,0755); r->first=now()-start; if(rc)goto out; leaf_created=true;
			r->id=cgid(leaf);if(!r->id)goto out;
			start=now(); rc=rmdir(leaf); r->second=now()-start;if(rc)goto out;leaf_created=false;
		...
		if(marker(markerfd,"END",op,family)||snapshot(statfd,taskfd,r,n))goto out;
		if(diag){
...
out:
	...
	printf("{\"schema\":\"cgroup-leaf-scx-control-v1\",\"scx_disabled\":%s,\"family\":%d,\"actor\":%d,\"n\":%d,\"same\":%d,\"callbacks\":%d,\"diagnostic\":%d,\"rounds\":%d,\"warmup\":%d,\"trace\":%s,\"root\":\"%s\",\"root_id\":%llu,\"a_id\":%llu,\"b_id\":%llu,\"restored\":true,\"exit_kind\":%d,\"tids\":[",
	...
	for(int i=0;i<completed;i++){struct result *r=&rows[i];
		printf("%s{\"op\":%d,\"warmup\":%s,\"rejected\":%s,\"first_ns\":%llu,\"second_ns\":%llu,\"id\":%llu,\"from\":%llu,\"to\":%llu,\"weight\":%d,\"init\":%u,\"exit\":%u,\"set_weight\":%u,\"prep\":%u,\"move\":%u,\"cancel\":%u,\"denied\":%u,\"errors\":%u,\"verified\":true,\"pairs\":[",
		       i?",":"",i+1,i-(int)negative<warmup?"true":"false",r->rejected?"true":"false",
		       (unsigned long long)r->first,(unsigned long long)r->second,(unsigned long long)r->id,
