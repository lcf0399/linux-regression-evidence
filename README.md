...
Index updated on 2026-09-01. The MSG_RING collaborator thread and v2 patch
results were refreshed on that date. The futex stable-queue status was checked live in Gmail
...
| [`io-uring-msg-ring-send-fd-install/`](io-uring-msg-ring-send-fd-install/) | Report sent on 2026-08-07 and publicly archived by the io-uring lore archive. A private collaborator, Uzair Beg, supplied revised patches; the latest validation reply was sent on 2026-09-01. This collaboration is not a maintainer resolution. | Exact A/B attributes an `11.621%` fixed-file installation slowdown to `7029acd8a950`. On v6.18-rc4, revised patch 1 was effectively neutral and the bulk-refill patch added `+0.007%` versus patch 1. An untimed probe confirmed the bulk path executed in full, so the neutral result is not a missed-path artifact. |
