...
BASE=${BASE:-$REMOTE_ROOT/linux-baremetal/mprotect-v713-shared-hint-20260722}
...
START_AT=${START_AT:-baseline-a}
HELPERS=./scripts/baremetal/kernel
POINT_RUNNER=${POINT_RUNNER:-./linux-regression-evidence/mprotect-shared-dirty-toggle/bare-metal/20260722-v713-shared-pte-hint-fastpath-ab/run_shared_hint_point.sh}
TRACE_RUNNER=${TRACE_RUNNER:-./linux-regression-evidence/mprotect-shared-dirty-toggle/bare-metal/20260722-v713-shared-pte-hint-fastpath-ab/run_lookup_trace_smoke.sh}

POINT_ROLES=(baseline candidate baseline)
POINT_LABELS=(baseline-a candidate baseline-b)

fail()
{
	echo "shared-hint remote matrix failed: $*" >&2
	exit 1
}
...
wait_for_new_boot()
{
	...
	while (( SECONDS < deadline )); do
		set +e
		# These substitutions are intentionally evaluated by the remote shell.
...
run_point()
{
	...
	if [[ "$label" != baseline-b ]]; then
		ssh_remote "cd '$REMOTE_ROOT' && '$TRACE_RUNNER' '$role' '$label'"
	fi
...
(( START_INDEX >= 0 )) || fail "START_AT must be one of: ${POINT_LABELS[*]}"
